...
class FindByReceiverImplTest {
    ...
    void setUp() {
        ...
        findByReceiver = new FindByReceiverImpl(gateway);
    }

    @Test
    void findNothing() {
        when(gateway.findById(1L)).thenReturn(Optional.empty());
        var messages = findByReceiver.query(1L);
    ...
    void findOptional() {
        ...
        when(gateway.findReceiverMessages(1L)).thenReturn(List.of(domain));
